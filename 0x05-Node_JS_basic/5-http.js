  ...
  return new Promise((resolve, reject) => {
    fs.readFile(path, 'utf8', (error, data) => {
      if (error) {
        ...
        // Filtering out empty lines and get student data
        const students = lines.filter((line) => line.trim() !== '').slice(1);

        // Creating a map to count students by field
        const fieldCount = {};
        ...
        // Resolving the promise with the output string
        resolve(output.trim());
      }
    });
  });
...
// Creating the HTTP server and assign it to the variable 'app'
