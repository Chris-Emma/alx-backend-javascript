...
function countStudents(databasePath) {
  return new Promise((resolve, reject) => {
    fs.readFile(databasePath, 'utf8', (error, data) => {
      if (error) {
        reject(new Error('Cannot load the database')); // Rejecting the promise if there's an error reading the file
      } else {
        const lines = data.split('\n');
        const students = lines.filter((line) => line.trim() !== '').slice(1); // Removing header and empty lines
...
// Makin the server listen on port 1245
const port = 1245;
app.listen(port, () => {
